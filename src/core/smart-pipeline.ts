// src/core/smart-pipeline.ts - Fix timing in smart pipeline execution
...
/*
    ...
    FIXED: Ultra-intelligent protection with accurate timing measurements
...
/**
 * Smart middleware with payload transformation
 */
const smartMiddlewareExecution = async (
  ctx: ProtectionContext,
  next: (ctx: ProtectionContext) => Promise<CyreResponse>
): Promise<CyreResponse> => {
  const {middlewares} = await import('../context/state')
  ...
  for (const middlewareId of ctx.action.middleware) {
    ...
    try {
      const result = await middleware.fn(ctx.action, currentPayload)
  ...
  ctx.transformedPayload = currentPayload
  return next(ctx)
}

/**
 * Build optimized protection strategy with caching
 */
export const buildSmartStrategy = (
  ...
  if (hotPath) {
    ...
    if (mask & PROTECTION_MASK.MIDDLEWARE)
      pipeline.push(smartMiddlewareExecution)
...
/**
 * FIXED: Execute smart pipeline with accurate timing and error tracking
 */
export const executeSmartPipeline = async (
  ...
  const strategy = buildSmartStrategy(action)
  ...
  if (strategy.hotPath) {
    ...
    // FIXED: Execute with proper timing measurement for hot path
    log.debug(`Ultra-hot path execution for ${action.id}`)

    try {
      // Measure execution time for hot path
      const executionStartTime = performance.now()
      const result = await finalExecution(payload)
      const executionEndTime = performance.now()
      const totalExecutionTime = executionEndTime - executionStartTime

      // Track execution metrics for hot path
      if (result.ok) {
      ...
      // FIXED: Track errors in hot path
  ...
  try {
    ...
    >(
      ...
      async (finalCtx: ProtectionContext) => {
        // FIXED: Measure execution time for cold path final execution
        const executionStartTime = performance.now()
        const result = await finalExecution(
          finalCtx.transformedPayload || finalCtx.payload
        )
        const executionEndTime = performance.now()
        const totalExecutionTime = executionEndTime - executionStartTime

        // Track execution metrics for cold path
        if (result.ok) {
          metricsReport.trackExecution(action.id, totalExecutionTime)
        }
    ...
    return await composedPipeline(ctx)
  } catch (error) {
    // FIXED: Track errors in cold path
    metricsReport.trackError(action.id)
    return {
      ok: false,
      payload: null,
