...
class Cyre {
  ...
  _action(id) {
    return this.emit(this.party[id].type, this.party[id].payload, this.party[id].repeat > 0 ? this._repeatAction(id) : this._taskComplete(id), this.party[id].group)
  }
  ...
  channel(x = {}) {
    if (this.party[x.id]) return console.error('@qi.channel: channel already exist', x.id)
