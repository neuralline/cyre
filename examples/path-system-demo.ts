...
async function securePathDemo() {
  ...
  // Medium operation with confirmation (should succeed)
  console.log('\n⚖️  Medium operation with confirmation:')
  const mediumBulk = await cyre.path.bulkCall(
    'building/**',
    {
      mediumTest: true,
      timestamp: Date.now()
    ...
    {
      confirmLargeOperation: false // Explicitly allow medium operations
    }
  )

  console.log(`   Result: ${mediumBulk.message}`)
  console.log(
    `   Success rate: ${mediumBulk.successfulCalls}/${mediumBulk.matchedChannels}`
  )
  ...
  const largeBlocked = await cyre.path.bulkCall(
    ...
    {
      // No force flag, no confirmLargeOperation: false - should be blocked
      maxChannels: 100
    }
  )
  ...
  console.log(
    '\n⚠️  Large operation with explicit force and confirmation disabled:'
  )
  const largeForced = await cyre.path.bulkCall(
    ...
    {
      force: true, // Explicit force required
      confirmLargeOperation: false, // Explicit confirmation disabled
      maxChannels: 100
    }
  )
